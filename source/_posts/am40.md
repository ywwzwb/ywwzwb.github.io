...
```
### 2.4 修改dtb
如果安装完系统后，机身的USB 只有一个可用，那么首先需要检查机内的调试开关是否切换到 `NORMAL` 。
...
### 4.1 准备模版
部署 lxc 容器需要准备一个模版，可以在这里下载，如果不想自己编译，可以在这里下载。[https://mirrors.ustc.edu.cn/openwrt/releases/23.05.5/targets/armsr/armv8/openwrt-23.05.5-armsr-armv8-rootfs.tar.gz](https://mirrors.ustc.edu.cn/openwrt/releases/23.05.5/targets/armsr/armv8/openwrt-23.05.5-armsr-armv8-rootfs.tar.gz).
...
### 4.2 创建虚拟网桥
机器自己只有一个网口，为了做软路由，我们需要再插一个 USB 网卡，这个网卡会被识别为 eth1.
外接的 USB 网卡作为wan 口，内置的网卡作为 lan  口。
...
创建一个网卡，eth0, 桥接到 lan. 更多网络设置，稍后还会再设置。
#### 4.3.2 修改虚拟机配置
虚拟机还需要调整一下配置
``` bash
